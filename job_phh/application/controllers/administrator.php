...
class Administrator extends CI_Controller{
	...
	public function berita(){
		...
		$limit = 15;
		$data['berita'] = $this->modeladmin->getAllBerita($offset, $limit);
		$data['count'] = $this->modeladmin->getAllBerita_count();
		...
		$data['judul'] = "berita | Administrator";
	...
	public function tambah_berita(){
		$data['error'] = "";
		$data['kategori'] = $this->modeladmin->getKategoriBerita();
		$data['judul'] = "Tambah Berita | Administrator";
		$this->load->view('template/admin/header', $data);
		$this->load->view('template/admin/nav');
	...
	public function submit_tambah_berita(){
	$this->form_validation->set_rules('nama_berita','Nama berita', 'required|xss_clean|max_length[255]|trim|strip_tags');
	...
	if($this->form_validation->run() == TRUE){ 
		  ...
		  $this->upload->initialize($config);
			if ($this->upload->do_upload('gambar_berita')) {
				...
				$this->_resize_berita($dok['file_name']);
				$this->_create_thumb_berita($dok['file_name']);
			$foto = $dok['file_name'];
			$kategori = $this->input->post('id_kategori');
			$input_nama_berita= $this->input->post('nama_berita');
			$input_deskripsi= $this->input->post('deskripsi');
			$ganti = array("'");
			$oleh = "&#039;";
			$nama_berita = str_replace($ganti, $oleh, $input_nama_berita);
			$url_title = url_title($nama_berita);
			$deskripsi = str_replace($ganti, $oleh, $input_deskripsi);
			
			$this->modeladmin->inputBerita($nama_berita, $url_title, $deskripsi, $foto, $kategori);
			
			$this->session->set_flashdata('info', "Berita berhasil ditambahkan.");
			redirect('administrator/berita');
			}else{
		...
		$data['error'] = $this->upload->display_errors();
		$data['kategori'] = $this->modeladmin->getKategoribBerita();
		$data['judul'] = "Tambah Berita | Administrator";
		$this->load->view('template/admin/header', $data);
		$this->load->view('template/admin/nav');
		$this->load->view('admin/berita/tambah_berita', $data);
	...
	public function edit_berita($id){
		$data['kategori'] = $this->modeladmin->getKategoriBerita();
		$data['edit_berita'] = $this->modeladmin->getEditBerita($id)->row();
		$data['judul'] = "Tambah Berita | Administrator";
		$this->load->view('template/admin/header', $data);
		$this->load->view('template/admin/nav');
	...
	public function submit_edit_berita(){
	...
	if($this->form_validation->run() == TRUE){ 
		   ...
		   $this->upload->initialize($config);
			...
			if ($this->upload->do_upload('gambar_berita')) {
			//unlink gambar
			$query = $this->modeladmin->getEditBerita($id)->row();						
			if ($query->file_name != "" || $query->file_name != NULL ){
				if(file_exists('./upload/berita/' . $query->file_name) || file_exists('./upload/berita/thumb/'. $query->file_name)) {
					$do = unlink('./upload/berita/'. $query->file_name); //menghapus gambar semula di folder
					$do = unlink('./upload/berita/thumb/'. $query->file_name); //menghapus gambar semula di folder
				}
			} 
				$dok = $this->upload->data();
				$this->_resize_berita($dok['file_name']);
			...
			$this->modeladmin->updateBeritaFoto($id, $nama_berita, $url_title, $deskripsi, $foto, $kategori);
			
			$this->session->set_flashdata('info', "Berita berhasil diubah, gambar diubah.");
			redirect('administrator/berita');
			}else{
				$kategori = $this->input->post('id_kategori');
				$input_nama_berita= $this->input->post('nama_berita');
				...
				$this->modeladmin->updateBeritaTanpaFoto($id, $nama_berita, $url_title, $deskripsi, $kategori);
				
				$this->session->set_flashdata('info', "Berita berhasil diubah, gambar tidak diubah.");
				redirect('administrator/berita');
			}	
		}  
		$data['kategori'] = $this->modeladmin->getKategoriBerita();
	...
	public function hapus_berita($id){
		...
		$this->modeladmin->hapus_berita($id);
	...
	public function kategori_berita(){
		$data['kategori'] = $this->modeladmin->getAllKategoriBerita();
		$data['judul'] = "Kategori Berita | Administrator";
		$this->load->view('template/admin/header', $data);
		$this->load->view('template/admin/nav');
	...
	public function submit_tambah_kategori(){
		...
		if($this->form_validation->run() == TRUE){
			$this->modeladmin->inputKategoriBerita($this->input->post('nama_kategori'));
		$this->session->set_flashdata('info', "Kategori Berita berhasil ditambahkan.");
		redirect('administrator/kategori_berita');
		}
		$data['kategori'] = $this->modeladmin->getAllKategoriBeritak();
		$data['judul'] = "Kategori Berita | Administrator";
		$this->load->view('template/admin/header', $data);
		$this->load->view('template/admin/nav');
		$this->load->view('admin/kategori/kategori', $data);
	...
	public function edit_kategori_berita($id){
		$data['edit_kategori'] = $this->modeladmin->getEditKategoriBerita($id)->row();
		$data['judul'] = "Edit Kategori Berita | Administrator";
		$this->load->view('template/admin/header', $data);
		$this->load->view('template/admin/nav');
	...
	public function submit_edit_kategori_berita(){
	$id = $this->input->post('id');
		$this->form_validation->set_rules('nama_kategori', 'Nama Kategori', 'required|xss_clean|max_length[255]|trim|strip_tags');
		if($this->form_validation->run() == TRUE){
			$this->modeladmin->updateKategoriBerita($this->input->post('id'), $this->input->post('nama_kategori'));
		$this->session->set_flashdata('info', "Kategori Berita berhasil diubah.");
		redirect('administrator/kategori_berita');
		}
		$data['edit_kategori'] = $this->modeladmin->getEditKategoriBerita($id)->row();
		$data['judul'] = "Edit Kategori Berita | Administrator";
	...
	public function hapus_kategori($id){
		...
		$query = $this->modeladmin->getBeritaByIdKategori($id)->row();						
			if ($query->file_name != "" || $query->file_name != NULL ){
				if(file_exists('./upload/berita/' . $query->file_name) || file_exists('./upload/berita/thumb/'. $query->file_name)) {
					$do = unlink('./upload/berita/'. $query->file_name); //menghapus gambar semula di folder
					$do = unlink('./upload/berita/thumb/'. $query->file_name); //menghapus gambar semula di folder
	...
	public function _resize_berita($fulpat) {
        $config['source_image'] = './upload/berita/' . $fulpat;
        $config['new_image'] = './upload/berita/' . $fulpat;
	...
	public function _create_thumb_berita($fulpet) {
        $config['source_image'] = './upload/berita/' . $fulpet;
        $config['new_image'] = './upload/berita/thumb/' . $fulpet;
        $config['maintain_ratio'] = TRUE;
        $config['width'] = 200;
        //$config['height'] = 200;
        $this->image_lib->initialize($config);
